...
func init() {
	...
	goodbyeCmd.Flags().StringVar(&options.RemoteAddr, "remote-addr", options.RemoteAddr, "the remote server's addr for this client to connect to")
	goodbyeCmd.Flags().StringVar(&options.HttpAddr, "http-addr", options.HttpAddr, "the remote http gateway addr")
	goodbyeCmd.Flags().BoolVar(&options.JsonLog, "json-log", options.JsonLog, "The format of the log is json or user friendly key-value pairs")
	goodbyeCmd.Flags().StringVar(&options.Name, "name", options.Name, "The name to send in Hello request")
	goodbyeCmd.Flags().Int32Var(&options.Age, "age", options.Age, "The age to send in Hello request")
	goodbyeCmd.Flags().StringVar(&options.Email, "email", options.Email, "The email to send in Hello request")
	goodbyeCmd.Flags().StringVar(&options.Address, "address", options.Address, "The address to send in Hello request")
	goodbyeCmd.Flags().Int64Var(&options.IntervalMilliSec, "interval-milli-sec", options.IntervalMilliSec,
		"The interval between two requests. Negative numbers mean sending one request.")
	goodbyeCmd.Flags().StringVar(&options.RgName, "rg-name", options.RgName, "The name of the resource group")
	goodbyeCmd.Flags().StringVar(&options.RgRegion, "rg-region", options.RgRegion, "The region of the resource group")
	goodbyeCmd.Flags().BoolVar(&options.CallAllRGOps, "call-all-rg-ops", options.CallAllRGOps, "Call all resource group operations")
}
